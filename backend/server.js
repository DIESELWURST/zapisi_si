...
// Endpoint to generate and send OTP for email verification
  ...
  connection.query(query, [otp, expiresAt, email], (err, results) => {
    ...
    sgMail.send(msg)
      ...
      .catch((error) => {
        ...
        return res.status(500).json({ error: 'Error sending OTP email', details: error });
      });
  });
});
...
app.post('/api/verify-otp', (req, res) => {
  ...
  connection.query(query, [email], (err, results) => {
    ...
    if (results.length === 0 || results[0].verification_code !== otp || new Date() > new Date(results[0].verification_expires)) {
